...
import { Upload, FileText, Download, Building2, Loader2, ShieldCheck } from 'lucide-react';
...
export function CorporateRegistration() {
  ...
  // Two distinct consents, matching the updated Appendix III form (Data
  // Protection Act 2019) - same split used on Minor Registration:
  // agreeDeclaration covers truthfulness + consent to data processing and
  // is required; agreeCommunications is a separate, optional consent to
  // being contacted about the account.
  ...
  const handleSingleFileUpload = (
    field: 'memberList' | 'registrationCertificate' | 'byLaws' | 'patronEndorsement',
    file: File | undefined
  ) => {
    if (!file) return;
  ...
  const validate = () => {
    ...
    // Patron's endorsement is "where applicable" on the form, i.e. optional - not validated here.

    // Communications consent is intentionally NOT validated here - it's an
    // optional, separate consent (see state comment above).
    if (!agreeDeclaration) newErrors.declaration = 'You must agree to the joint declaration and consent to proceed';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitCorporateRegistration({
        formData,
        isChurchGroup,
        signatories,
  ...
  return (
    <div className="min-h-screen bg-[#FAF9F5] font-sans py-12 md:py-20">
      {/* Full-bleed page like the rest of the site (About Us, Products,
          Register) rather than a flat max-w-* wrapper - the outer padding
          runs edge to edge, but the form itself is placed within a centered
          span of the 12-column grid so long text inputs don't stretch to an
          unreadable width on a big monitor. */}
      {/* Full-bleed page like the rest of the site (About Us, Products,
      ...
      <div className="mx-auto px-6 sm:px-8 lg:px-12 xl:px-20">
        <div className="text-center mb-12">
          ...
          <div className="mt-4">
            <a
              href="/New Corporate Application Form.pdf"
        ...
        <form onSubmit={handleSubmit} className="space-y-14">
              <Notice>
                ...
                your group's By-laws/Constitution, and your Patron's endorsement (where applicable).
              </Notice>
              ...
              <div className="border-t-2 border-[#6B9E4D] pt-8">
                ...
                <div className="space-y-5">
                  ...
                  {/* NEW - "Patrons' endorsement, where applicable" is item 6 in the
                      updated form's Requirements list. The API already had a slot
                      for this (patronEndorsement / patron_endorsement_path) but the
                      form never actually exposed a way to upload one - added here. */}
                  <div>
                    <div className="border border-dashed border-[#6B9E4D] p-4 hover:bg-[#F3F0E8] transition-colors">
                      <Label htmlFor="patronEndorsement" className="cursor-pointer">
                        <div className="flex items-center space-x-3">
                          <Upload className="text-[#237A17]" size={20} strokeWidth={1.5} />
                          <div className="flex-1">
                            <p className="text-[#16210E]">Patron's Endorsement (where applicable)</p>
                            <p className="text-base lg:text-lg text-gray-500">JPG, PNG, or PDF - Max 5MB</p>
                          </div>
                          {uploadedFiles.patronEndorsement && <FileText className="text-[#237A17]" size={20} />}
                        </div>
                      </Label>
                      <Input
                        id="patronEndorsement"
                        type="file"
                        accept="image/jpeg,image/png,image/jpg,application/pdf"
                        onChange={(e) => handleSingleFileUpload('patronEndorsement', e.target.files?.[0])}
                        className="hidden"
                      />
                      {uploadedFiles.patronEndorsement && (
                        <p className="text-base lg:text-lg text-[#237A17] mt-2">&#10003; {uploadedFiles.patronEndorsement.name}</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Declaration & Data Protection Consent */}
              <div className="border-t-2 border-[#16210E] pt-8">
                <SectionHeading eyebrow="Final Step" title="Joint Declaration & Data Protection Consent" />
                <div className="space-y-5">
                  <p className="text-base lg:text-lg text-gray-700 leading-relaxed">
                    We jointly and severally declare that all the information given herein is true to the best of
                    our knowledge and shall abide by all the terms and conditions laid down by the self-help
                    group. (Note: Giving false information is an offence under the laws of Kenya.) We acknowledge
                    that St Gabriel Catholic Church SHG complies with the Data Protection Act, 2019, and will
                    handle our personal data responsibly.
                  </p>
                  ...
                  <div className="flex items-start space-x-2 pt-2">
                    ...
                    <Label htmlFor="agreeDeclaration" className="text-base lg:text-lg text-gray-700 cursor-pointer leading-relaxed">
                      On behalf of the group's signatories, I/we consent to the collection, processing, and
                      storage of our personal information for account management and compliance with legal
                      obligations. I/we understand we have the right to be informed, access, amend, object to, or
                      request deletion of this data as permitted by applicable law. *
                    </Label>
                  </div>
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <Label htmlFor="agreeCommunications" className="text-base lg:text-lg text-gray-700 cursor-pointer leading-relaxed">
                      I/we consent to receive communication regarding our account and related services via
                      e-mail, phone, or any other applicable channel.{' '}
